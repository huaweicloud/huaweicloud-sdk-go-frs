...
func CanonicalHeaders(r *http.Request, signerHeaders []string) string {
	...
	for _, key := range signerHeaders {
		...
		var q []string
		for _, v := range value {
			q = append(q, strings.TrimSpace(v))
		}
		a = append(a, key+":"+strings.Join(q, ","))
	}
	return fmt.Sprintf("%s\n", strings.Join(a, "\n"))
}
...
func (s *Signer) Sign(r *http.Request) error {
	...
	signature, err := SignStringToSign(stringToSign, []byte(s.AppSecret))
	if err != nil {
		return err
	}
	authValue := AuthHeaderValue(signature, s.AppKey, signedHeaders)
